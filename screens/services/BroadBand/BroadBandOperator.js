...
const BroadBandOperator = ({ navigation, route }) => {
...
let countryCode = '+91'
  ...
  const Data = () => {
    ...
    fetch(`${REACT_APP_BASE_Bill_URL}/getBroadbandPostpaidBillers`)
  ...
  const postUserData = () => {
    setLoading(true)
    fetch(`${REACT_APP_BASE_Bill_URL}/BroadbandPostpaidfetchBill`, {
      method: 'POST', 
      body: JSON.stringify({ 
        mobile: mobileNo,
        ip: ipId,
        mac: 'B8-8A-60-9E-F2-BE',
        billerId: datas,
      }),
      headers: {
      ...
      .then((data) => {
       ...
       // console.log('resp is ----', data.data.result.payload);
        ...
        if (data.status === true) { 
          ...
          setLoading(false) 
          navigation.navigate("UserBillBroadBand", {
            'ipID': ipIDis,
            'macid': macIDis,
            'mobileNo': mobileNo,
            'billerId':datas, 
    ...
    <SafeAreaView>
      ...
      <View style={{ backgroundColor: '#132fba', borderBottomLeftRadius: 10, borderBottomRightRadius: 10, marginLeft: 0.5, marginRight: 0.5 }}>
        <View style={{ flexDirection: 'row', padding: 15, }}>
          ...
          <Text style={{ fontSize: 20, fontWeight: '400', justifyContent: 'center', textAlign: 'center', color: 'white', marginLeft: '20%' }}>BroadBand Operator</ Text>
        </View>
      ...
      <View style={{ marginTop: '4%', padding: 20, }}>
        <Text style={{ fontSize: 20, padding: 8, fontWeight: '400', color: 'black', marginTop: '1%', marginBottom: -13 }}> Enter Customer Id / Mobile No </ Text>
        <View style={{ flexDirection: 'row', marginTop: 10, borderBottomWidth: 1, borderColor: 'black' }}>
          ...
          <TextInput placeholder='0000000000' onChangeText={(text) => {setMobileNo(text) }} keyboardType="numeric" maxLength={10} style={{ fontSize: 24, borderWidth: 0,width:'100%', marginLeft: 14, marginTop: 5, color: 'black' }} />
        </View>
      </View>
      <View style={{}}>
        <View style={{}}>
