...
const FastTagBillFetch = ({ navigation, route }) => {
  const { ipID, macid, mobileNo, billerId, biller_name, billDate, dueDate,vachileNo, refId, billerTotalAmount, billerAccountNo } = route.params;
  ...
  const postUserData = () => {
    setLoading(true)
    fetch(`${REACT_APP_BASE_Bill_URL}/fastagBillPaymentRequest`, {
      method: 'POST',
      body: JSON.stringify({
    ...
    <SafeAreaView style={{ backgroundColor: "#F9F6EE", flex: 1 }}>
      <View style={{ backgroundColor: '#132fba', borderBottomLeftRadius: 15, borderBottomRightRadius: 15, marginLeft: 0.5, marginRight: 0.5 }}>
        <View style={{ flexDirection: 'row', padding: 15, }}>
          ...
          <Text style={{ fontSize: 20, fontWeight: '400', justifyContent: 'center', textAlign: 'center', color: 'white', marginLeft: '20%' }}>FastTag  BILLS</ Text>
        </View>
...
export default FastTagBillFetch