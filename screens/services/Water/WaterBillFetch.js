...
const WaterBillFetch = ({ navigation, route }) => {

  const { ipID, macid, mobileNo, billerId, biller_name, billDate, dueDate, refId, billerTotalAmount, customer_ID } = route.params;
  const [loading, setLoading] = useState(false);

  console.log('=imei============>', JSON.stringify(refId))
  console.log('=======ip is======>', JSON.stringify(billerId))

  // ----------------- POST API CALL UserData Send ----------------------------

  const waterpayment = () => {
    setLoading(true)
    fetch(`${REACT_APP_BASE_Bill_URL}/waterBillPaymentRequest`, {
      method: 'POST',
      body: JSON.stringify({
      ...
      .then((data) => {
        ...
        console.log('updated---', data.data.txnId);
        if (data.data.status === 200) {
          navigation.navigate("WaterpaymentScreen", {
            'mobileNo': mobileNo,
    ...
    <SafeAreaView style={{ backgroundColor: "#F9F6EE", flex: 1 }}>
      <View style={{ backgroundColor: '#132fba', borderBottomLeftRadius: 15, borderBottomRightRadius: 15, marginLeft: 0.5, marginRight: 0.5 }}>
        <View style={{ flexDirection: 'row', padding: 15, }}>
          ...
          <Text style={{ fontSize: 20, fontWeight: '400', justifyContent: 'center', textAlign: 'center', color: 'white', marginLeft: '20%' }}>Water  BILLS</ Text>
        </View>
      </View> 
      <View>
        ...
        <View style={styles.modalView}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: '2%' }}>
            <Text style={{ fontSize: 16, color: 'black' }}>Consumer id</Text>
            <Text style={{ fontSize: 16, color: 'black' }}>{customer_ID} </Text>
          </View>
